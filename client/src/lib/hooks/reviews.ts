...
export function usePrReviews(prId: string | null | undefined, options?: { enabled?: boolean }) {
  return useQuery({
...
export function useDeleteRun(prId: string | null | undefined) {
  ...
  return useMutation({
    ...
    // both the timeline and the Review Runs list from cache — plus Smart Diff,
    // whose markers are backed by the same findings.
    onSuccess: () => {
