...
/**
 * L02 — the skills lesson's agent.
 *
 * Deliberately holds the ROLE and the output conventions but NOT the concrete
 * checklists: what counts as an uncovered branch, which corner cases matter,
 * where mocking stops being useful and what makes a test flaky all live in the
 * skills linked to this agent (`test-coverage-rubric`, `edge-case-checklist`,
 * `mock-discipline`). That split is the point of the lesson — the same agent,
 * run with its skills off, misses what it catches with them on.
 */
