...
async function writeFileAt(root: string, rel: string, contents: string): Promise<void> {
  ...
  // `dirname`, not a manual '/' search: `join` normalises separators to '\' on
  // win32, so scanning for a forward slash found nothing there, the parent dir
  // was never created, and every nested-path case failed with ENOENT.
