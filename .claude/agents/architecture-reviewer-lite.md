...
description: Relaxed A/B variant of `architecture-reviewer`, used only by the eval suite to measure what the "name the documented rule for every finding" requirement is worth. Identical in every other respect — same scope, same checks, same read-only tool set. Do NOT use this for real reviews; `architecture-reviewer` is the production agent. Kept in `.claude/agents/` because `evals/agents/architecture-reviewer-lite/` loads it by path.
