...
export interface ReviewInput {
  ...
  memory?: string[];
  /**
   * Resolved project-context documents (own + skill-inherited attachments),
   * full text as read from the repository's working copy (untrusted;
   * delimiter-wrapped downstream into ONE `project-context` block, path
   * stated before text).
   */
  ...
  /**
   * Derived PR intent/scope (L03 intent layer; untrusted). Passed straight
   * through to `assemblePrompt` — omitted (and the `SCOPE_DISCIPLINE` system
   * rule not appended) when absent or its `intent` string is blank.
   */
  intent?: Intent;
